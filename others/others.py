...
def get_app_data_dir():
    ...
    Returns:
        string: Ścieżka do katalogu %APPDATA%/SafePad
    """
    app_data = os.environ.get('APPDATA', '')
    if not app_data:
        app_data = os.path.expanduser('~')
    
    return os.path.join(app_data, 'SafePad')
...
class Argon2Benchmark:
    ...
    def save_benchmark_results(cls, registry_conf, results):
        """
        Zapisuje wyniki benchmarku do rejestru.
        ...
        try:
            import winreg
            key = winreg.CreateKey(winreg.HKEY_CURRENT_USER, registry_conf.REG_PATH)
            winreg.SetValueEx(key, "last_benchmark_time", 0, winreg.REG_SZ, 
                            datetime.now().isoformat())
            winreg.SetValueEx(key, "last_benchmark_params", 0, winreg.REG_SZ, 
                            json.dumps(results))
            winreg.CloseKey(key)
        except:
            pass
...
def is_benchmark_needed(registry_conf, max_age_days=30):
    """
    Sprawdza czy potrzebny jest nowy benchmark.
    ...
    try:
        import winreg
        key = winreg.OpenKey(winreg.HKEY_CURRENT_USER, registry_conf.REG_PATH, 0, winreg.KEY_READ)
        
        try:
            last_time_str, _ = winreg.QueryValueEx(key, "last_benchmark_time")
